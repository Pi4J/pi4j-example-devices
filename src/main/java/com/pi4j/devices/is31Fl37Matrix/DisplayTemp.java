...
public class DisplayTemp {
    ...
    public DisplayTemp(Context pi4j, Console console, Is31Fl37Matrix display, Logger logger) {
        ...
        public void process_bmp_data(ControlLeds pin_monitor, Integer bus_num, Integer address, int led_blink,
            ...
            this.logger.trace("parms :  bus_num : " + String.format("0x%02X", bus_num) + " address : "
                    + String.format("0x%02X", address) + " blink : " + String.format("0x%02X", led_blink) + " loop_count : "
                    + String.format("0x%02X", loop_count) + " bmp180 address : " + String.format("0x%02X", bmp_address));

            BMP280Device bmpDev = new BMP280Device(this.pi4j, this.console, bmp_bus, bmp_address, this.logger);
            ...
            if (completed.getSuccessVal()) {
                ...
                pin_monitor.flash_alarm_led(true, this.display);
            }
            this.logger.trace("process_bmp_data");

        }

        public void show_time(ControlLeds pin_monitor, Integer bus_num, Integer address, int led_blink,
                              Integer loop_count) {
            int display_num = 42;
