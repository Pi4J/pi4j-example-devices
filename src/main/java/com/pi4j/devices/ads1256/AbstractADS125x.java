...
public abstract class AbstractADS125x implements ADS125x {
    ...
    public AbstractADS125x(Context pi4j, SpiBus spiBus, int chipSelect, boolean reset, int drdyPin, int csPin, int rstPin, boolean crtRstGpio, int pdwnPin, boolean crtPdwnGpio, Console console, String traceLevel, double vref, DataRate sampleRate, boolean useBuffer) throws InterruptedException {
        super();
    ...
    private void init() throws InterruptedException {
        ...
        var drdyConfig = DigitalInput.newConfigBuilder(pi4j)
            ...
            .name("DRDY")
            .bcm(this.drdyPinNum)
            .debounce(500L); //               .pull(PullResistance.PULL_UP)
        try {
        ...
        */
        ...
        drdyBarrier.setThrowableHandler((t) -> {
            t.printStackTrace();
            System.exit(1);
        });
        ...
        drdyBarrier.setAwaitToggleConflictHandler((barrier, state, state1) -> {
            return false;
        });
    ...
    public void configADC(Gain gain, DataRate drate, boolean autoCalibrate, boolean useBuffer) throws InterruptedException {
        ...
        sleepMS(1);
        try {
            drdyBarrier.awaitAnticipatedToggles(false, 250);
    ...
    private byte readRegData(int reg) {
        try {
            drdyBarrier.awaitAnticipatedToggles(true, 250);
