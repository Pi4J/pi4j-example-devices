...
public class ADS1256App {
	...
	for (int i = 0; i < args.length; i++) {
		...
		runChaser = true;
		try {chaseIntervalMS = Integer.parseInt(args[i+1]);i++;}
		catch(NumberFormatException | ArrayIndexOutOfBoundsException e) {}//Assume interval not supplied
	    } else if (o.contentEquals("-h")) {
		console.println(helpString);
		System.exit(44);
		...
		try {sampleRate = DataRate.valueOf(a);}
